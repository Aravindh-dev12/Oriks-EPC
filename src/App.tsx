...
function Home() {
  return (
    <>
      <HeroVideo />
      <section className="intro"><div className="wrap split"><div><span className="kicker">ORIKS ENERGY CARE</span><h2>Power infrastructure built around the point where energy becomes usable.</h2></div><p>Engineering, procurement, construction, evacuation, substations, transmission, commissioning and lifecycle support for renewable-power projects.</p></div></section>
      ...
      <section className="wrap feature"><div className="feature-media"><img src="https://images.unsplash.com/photo-1473341304170-971dccb5ac1e?auto=format&fit=crop&w=1800&q=88" alt="Power transmission infrastructure" /></div><div><span className="kicker">FROM GENERATION TO GRID</span><h2>Make the electrical path visible.</h2><p>ORIKS is structured around the systems that connect renewable generation to a reliable grid interface — from collection and transformation to transmission, protection, testing and handover.</p><Link className="button" to="/services">View capability map <Arrow /></Link></div></section>
...
function About() {
  return <Page title="About ORIKS" lead="A focused engineering company for renewable and power-infrastructure interfaces." image="https://images.unsplash.com/photo-1581094794329-c8112a89af12?auto=format&fit=crop&w=1800&q=88"><Section eyebrow="Our focus" title="Engineering discipline from generation to grid." /><p className="lead-copy">ORIKS Energy Care Private Limited is based in Karur, Tamil Nadu. The company is positioned around renewable EPC and electrical infrastructure: power evacuation, substations, transmission, testing, commissioning and lifecycle support.</p></Page>;
}
...
function Page({ title, lead, image, children }: { title: string; lead: string; image: string; children: ReactNode }) {
  return <><section className="inner-hero"><div className="wrap inner-grid"><div><span className="kicker">ORIKS ENERGY CARE</span><h1>{title}</h1><p>{lead}</p></div><img src={image} alt="" /></div></section><main className="wrap page-body">{children}</main></>;
}
